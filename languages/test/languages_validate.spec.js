...
describe('Validate', function () {
  ...
  const languagesSchemaPath = resolve(__dirname, '../languages.json');
  const localSchema = JSON.parse(readFileSync(schemaPath, 'utf-8'));
  const languagesSchema = JSON.parse(readFileSync(languagesSchemaPath, 'utf-8'));

  it('should validate schema', () => {
    const validateSchema = createAjvInstance().addSchema(languagesSchema).compile(localSchema);
